...
unsigned char i2cdata[1024]; //FIXBUG
...
//  uchar* data;
  ...
  while((nmax>0)&&(read_i2c(addr_i2c,i2cdata,bufsize)==FALSE)){ nmax--;__no_operation(); }
  // Retour pour le ByteCode
  VPUSH(PNTTOVAL(VMALLOCSTR((unsigned char*)i2cdata,bufsize)));
  //return nmax;
}
...
int sysI2cWrite(unsigned char addr_i2c, unsigned char *data, unsigned int bufsize)
{
  ...
  //while((nmax>0)&&(write_i2c(addr_i2c,zz,bufsize)==FALSE)){ nmax--;__no_operation(); }
